...
/* ------------------------------------------------------------------ *
 * Eraser export. The grammars are near-identical; Eraser wants `icon:` and
 * has no `kind:`, so we map kinds to its closest generic icon.
 * ------------------------------------------------------------------ */

const ERASER_ICON: Record<string, string> = {
  client: "users",
...
export function toEraser(diagram: Diagram): string {
  const out: string[] = [`direction ${diagram.direction}`, ""];
  const byGroup = new Map<string | undefined, DiagramNode[]>();
  for (const n of diagram.nodes) {
    const list = byGroup.get(n.group) ?? [];
    list.push(n);
  ...
  const children = new Map<string | undefined, DiagramGroup[]>();
  for (const g of diagram.groups) {
    const list = children.get(g.parent) ?? [];
    list.push(g);
    children.set(g.parent, list);
  }
  const emit = (scope: string | undefined, indent: string) => {
    for (const n of byGroup.get(scope) ?? []) {
      const icon = ERASER_ICON[n.kind] ?? "server";
      const label = n.label ? `, label: ${quote(n.label)}` : "";
      out.push(`${indent}${n.id} [icon: ${icon}${label}]`);
    }
    for (const g of children.get(scope) ?? []) {
      const label = g.label ? ` [label: ${quote(g.label)}]` : "";
      out.push(`${indent}${g.id}${label} {`);
      emit(g.id, indent + "  ");
      out.push(`${indent}}`);
    }
  };
  ...
  for (const e of diagram.edges) {
    ...
    out.push(`${e.from} ${CONNECTOR_TOKEN[e.style]} ${e.to}${label}`);
  }
  return out.join("\n") + "\n";
}
